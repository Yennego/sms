...
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await fetch(`${baseUrl}/academics/teacher-subject-assignments/bulk-delete`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ message: 'Failed to bulk delete assignments' }));
        ...
        console.error('[Teacher Subject Assignments Bulk Delete API] Error:', error);
