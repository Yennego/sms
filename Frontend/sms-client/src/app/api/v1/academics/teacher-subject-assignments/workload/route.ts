...
export async function GET(request: NextRequest) {
    try {
        ...
        const response = await fetch(`${baseUrl}/academics/teacher-subject-assignments/workload?${queryString}`, {
            method: 'GET',
            headers: {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ message: 'Failed to fetch teacher workload' }));
        ...
        console.error('[Teacher Subject Assignments Workload API] Error:', error);
