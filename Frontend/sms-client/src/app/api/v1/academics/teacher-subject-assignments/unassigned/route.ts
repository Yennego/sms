...
export async function GET(request: NextRequest) {
    try {
        ...
        const response = await fetch(`${baseUrl}/academics/teacher-subject-assignments/unassigned?${queryString}`, {
            method: 'GET',
            headers: {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({ message: 'Failed to fetch unassigned subjects' }));
        ...
        console.error('[Teacher Subject Assignments Unassigned API] Error:', error);
