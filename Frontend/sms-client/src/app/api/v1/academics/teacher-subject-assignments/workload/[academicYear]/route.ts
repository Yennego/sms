...
export async function GET(
  ...
  try {
    ...
    console.log('[Teacher Subject Assignments API] Processing GET workload request for academic year:', academicYear);
    ...
    console.log(`[Teacher Subject Assignments API] Sending workload request to backend with X-Tenant-ID: ${tenantId}`);

    const baseUrl = normalizeBaseUrl(process.env.BACKEND_API_URL);
    const { signal, cancel } = createTimeoutSignal(30_000);
    const response = await fetch(`${baseUrl}/academics/teacher-subject-assignments/workload/${academicYear}`,
      {
        method: 'GET',
        headers: {
    ...
    console.log('[Teacher Subject Assignments API] Successfully fetched teacher workload');
