...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${baseUrl}/academics/teacher-subject-assignments/all?${queryString}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ message: 'Failed to fetch all assignments' }));
    ...
    console.error('[Teacher Subject Assignments All API] Error:', error);
