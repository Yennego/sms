...
export async function PUT(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${baseUrl}/academics/teacher-subject-assignments/reassign?${queryString}`, {
      method: 'PUT',
      headers: {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ message: 'Failed to reassign teacher' }));
    ...
    console.error('[Teacher Subject Assignments Reassign API] Error:', error);
