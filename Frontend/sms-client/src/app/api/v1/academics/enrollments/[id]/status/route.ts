...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const backendUrl = `${process.env.BACKEND_API_URL}/academics/enrollments/${id}/status`;
  ...
  if (!response.ok) {
    ...
    return NextResponse.json(errorData || { error: 'Failed to update enrollment status' }, { status: response.status });
  }
