...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${baseUrl}/academics/academic-years/current/semester`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ message: 'Failed to fetch current semester' }));
    ...
    console.error('Error in current semester API route:', error);
