...
async def create_attendance(
    *,
    ...
    try:
        return await attendance_service.create(obj_in=attendance_in)
...
async def generate_attendance_report(
    *,
    ...
    return await attendance_service.generate_report(
        class_id=class_id,
        student_id=student_id,
        start_date=start_date,
        end_date=end_date,
...
async def get_absent_students(
    *,
    ...
    return await attendance_service.get_absent_students(
        class_id=class_id,
...
async def get_late_students(
    *,
    ...
    return await attendance_service.get_late_students(
        class_id=class_id,
        date=attendance_date or date.today()
    )

@router.get("/attendance/global-stats", response_model=dict)
async def get_global_attendance_stats(
    *,
    ...
    return await super_admin_service.get_global_stats(
        start_date=start_date,
        end_date=end_date
    )
...
async def get_student_attendance_percentage(
    *,
    ...
    percentage = await attendance_service.get_student_attendance_percentage(
        student_id=student_id,
        start_date=start_date,
        end_date=end_date
    )
...
async def get_class_attendance_trends(
    *,
    ...
    return await attendance_service.get_class_attendance_trends(
        class_id=class_id,
        days=days
    )
...
async def get_attendance(
    *,
    ...
    attendance = await attendance_service.get(id=attendance_id)
    if not attendance:
        raise HTTPException(
...
async def update_attendance(
    *,
    ...
    try:
        ...
        return await attendance_service.update(id=attendance_id, obj_in=attendance_in)
...
async def delete_attendance(
    *,
    ...
    return await attendance_service.delete(id=attendance_id)
...
async def mark_daily_attendance(
    *,
    ...
    try:
        return await attendance_service.mark_daily_attendance(
            student_id=student_id,
            class_id=class_id,
            schedule_id=schedule_id,
            academic_year_id=academic_year_id,
...
async def bulk_mark_attendance(
    *,
    ...
    try:
        # Set marked_by for the entire bulk operation
        attendance_in.marked_by = current_user.id
        
        return await attendance_service.bulk_mark_attendance(attendance_in)
    except ValueError as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
