...
def create_academic_grade(
    *,
    ...
    try:
        return grade_service.create(obj_in=grade_in)
...
def get_academic_grades(
    *,
    ...
    if is_active is not None and is_active:
        return grade_service.get_active_grades(skip=skip, limit=limit)
    else:
        filters = {}
        if is_active is not None:
            filters["is_active"] = is_active
        return grade_service.list(skip=skip, limit=limit, filters=filters)
...
def get_academic_grade(
    *,
    ...
    grade_obj = grade_service.get(id=grade_id)
    if not grade_obj:
        raise HTTPException(
...
def get_academic_grade_by_name(
    *,
    grade_service: AcademicGradeService = Depends(),
    name: str,
    current_user: User = Depends(has_any_role(["admin", "teacher", "student"]))
    ...
    grade_obj = grade_service.get_by_name(name=name)
    if not grade_obj:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
def update_academic_grade(
    *,
    ...
    try:
        ...
        return grade_service.update(id=grade_id, obj_in=grade_in)
    except DuplicateEntityError as e:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=str(e)
...
def delete_academic_grade(
    *,
    ...
    return grade_service.delete(id=grade_id)